...
class MarvDatasetFlipperEnv:
    def __init__(self, max_datasets=100, dir_name="sm"):
        ...
        # Load all datasets
        self.data_dict_list = []
        dataset_dir = os.path.join(os.path.dirname(__file__), "supervised_dataset/{}".format(dir_name))
    ...
    def get_random_batch_feature_vec_haar(self, batchsize, loss_mask_list=None):
        rnd_indeces = np.random.choice(np.where(loss_mask_list == 0)[0], batchsize, replace=False)
        feature_list = []
        label_list = []
        for ri in rnd_indeces:
            o = self.data_dict_list[ri]
            feature_list.append(o["haar_feats"])
            label_list.append(self.state_list.index(self.short_to_state_name_dict[o["teleop_state"]]))
        feature_arr = T.tensor(feature_list)
        label_arr = T.tensor(label_list)
...
if __name__=="__main__":
    MarvDatasetFlipperEnv(max_datasets=1, dir_name="sm_full") # HERE ITS MAX 1 DATASET FOR DEBUGGING PURPOSES, DONT CHANGE. CHANGE IT IN train_sm_flip....
