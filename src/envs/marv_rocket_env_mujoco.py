...
class MARVRocketBalMujocoEnv:
    ...
    def get_obs_new(self):
        # Torso xyz and orientation
        body_pos = self.sim.data.body_xpos[self.geom_name_dict["body"]]
        body_quat = self.sim.data.body_xquat[self.geom_name_dict["body"]]
        body_mat = self.sim.data.body_xmat[self.geom_name_dict["body"]]

        body_vel = self.sim.data.body_xvel[self.geom_name_dict["body"]]
        body_velr = self.sim.data.body_xvelr[self.geom_name_dict["body"]]
    ...
    def step(self, act):
        ...
        #target_height, target_pitch = self.calc_target_height_and_pitch()

        tail_height = self.sim.data.geom_xpos[self.geom_name_dict["tail"]][2]
        p_sc = (self.param_target + 1) * 0.5
        target_tail_height = 0.174 + p_sc * 0.38
        #1 / (1 + 20 * abs(pitch - target_pitch))
        #rew = 1 / (1 + 80 * abs(tail_height - target_tail_height)) + 1 / (1 + 10 * np.square(obs[0])) - np.maximum(pitch, 0) * 3
        #rew = verticality + (tail_height - 0.174) * 10 + 1 / (1 + 10 * np.square(obs[0])) - np.maximum(pitch, 0) * 3 # Don't touch, it's for full rocket
        rew = 1 / (1 + 100 * np.square(tail_height - target_tail_height)) + 1 / (1 + 10 * np.square(obs[0])) - np.maximum(pitch, 0) * 1

        toppled = pitch > 2
        out_of_bnds = abs(obs[0]) > 1.5
