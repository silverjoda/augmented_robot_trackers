...
from geometry_msgs.msg import Twist, Vector3, Pose, PoseArray, PoseStamped
...
class SpotAssitant:
    def __init__(self, config):
        self.config = config
        self.crawl_mode_on = self.config["permanently_force_crawl_mode"]
    ...
    def init_ros(self, name):
        ...
        self.debug_track_lock = threading.Lock()
        self.trav_vis_pc_lock = threading.Lock()
        self.nav_cmd_vel_lock = threading.Lock()
        self.spot_cmd_vel_lock = threading.Lock()
        ...
        self.bahavior_fault_lock = threading.Lock()
        ...
        rospy.Subscriber("/spot/cmd_vel",
                         Twist,
                         self._ros_spot_cmd_vel_callback, queue_size=1)

        rospy.Subscriber("/rds/debug_track",
                         Path,
                         self._ros_debug_track_callback, queue_size=1)
        ...
        self.force_crawl_enabling_service = rospy.Service("force_crawl_service", SetBool,
                                                        self.force_crawl_handler)
        ...
        self.combined_path_pub = rospy.Publisher(self.config["combined_path_topic"],
                                                     Path,
                                                     queue_size=1)

        time.sleep(1)

    def force_crawl_handler(self, input_val):
        self.crawl_mode_on = input_val.data
        return True, "ok"

    def _ros_debug_track_callback(self, data):
        with self.debug_track_lock:
            self.debug_track_data = data
            with self.path_lock:
                if hasattr(self, "path_data") and self.path_data is not None:
                    self.combined_path = self.merge_paths(deepcopy(self.debug_track_data), deepcopy(self.path_data))

    def _ros_behaviorfaultstate_callback(self, data):
        with self.bahavior_fault_lock:
            # Check for Behavior Faults, if any, just clear them, lol.
            attempt_to_self_right = False
            if len(data.faults) > 0:
                for bf in data.faults:
                    # Get ID of fault, and then clear it if possible
                    if bf.status == 1:
                        if bf.cause == 1:
                           attempt_to_self_right = True
                        self.clear_behavior_fault(bf.behavior_fault_id)

            if attempt_to_self_right:
                ...
                if hasattr(self, "feedback_data") and self.feedback_data is not None:
                    for i in range(3):
                        ...
                        time.sleep(2)
                        if self.feedback_data.standing or self.feedback_data.sitting:
                            break
                self.call_service("stand")
                time.sleep(3)

                if self.feedback_data.standing:
                    rospy.loginfo("Spot might be back up..")
                ...
                if bf.cause == 1 and bf.status == 2:
                    ...
                    rospy.logerr_throttle(1, "Spot has fallen and has an unrecoverable fault :(  ... only a platform restart will work")
    ...
    def _ros_spot_cmd_vel_callback(self, data):
        with self.spot_cmd_vel_lock:
            self.spot_cmd_vel_data = data
    ...
    def merge_paths(self, debug_track, path):
        def poses_identical(p1, p2):
            return np.sum([np.abs(p1.pose.position.x - p2.pose.position.x),
                           np.abs(p1.pose.position.y - p2.pose.position.y),
                           np.abs(p1.pose.position.z - p2.pose.position.z)]) < 0.01

        if len(debug_track.poses) == 0:
            return self.fix_path(path)
        elif len(path.poses) == 0:
            return self.fix_path(debug_track)
        else:
            merged_poses = deepcopy(debug_track.poses)
            # Go over path points and stop once it matches the last point of the debug track
            ctr = 0
            for p in path.poses:
                if poses_identical(p, debug_track.poses[-1]):
                    break
                ctr += 1

            for i in range(ctr + 1, len(path.poses)):
                merged_poses.append(path.poses[i])

            combined_path = Path()
            combined_path.header.stamp = debug_track.header.stamp
            combined_path.header.frame_id = debug_track.header.frame_id
            combined_path.poses = merged_poses
            fixed_combined_path = self.fix_path(combined_path)
            return fixed_combined_path

    def fix_path(self, path):
        def dist_between_poses(p1, p2):
            d = np.sqrt(np.sum(np.square(np.array([p1.pose.position.x, p1.pose.position.y, p1.pose.position.z]) -
                                     np.array([p2.pose.position.x, p2.pose.position.y, p2.pose.position.z]))))
            return d

        fixed_path = deepcopy(path)

        dist_per_pt = 0.2
        for i in range(len(path.poses) - 1):
            pose_1, pose_2 = path.poses[i : i + 2]
            dbp = dist_between_poses(pose_1, pose_2)
            if dbp > (dist_per_pt * 2):
                n_additional_pts = int(dbp / dist_per_pt) - 1
                for j in range(1, 1 + n_additional_pts):
                    new_point = (1. - (float(j) / (n_additional_pts + 2))) * np.array([pose_1.pose.position.x, pose_1.pose.position.y, pose_1.pose.position.z]) + \
                                (float(j) / (n_additional_pts + 2)) * np.array([pose_2.pose.position.x, pose_2.pose.position.y, pose_2.pose.position.z])
                    new_pose = PoseStamped()
                    new_pose.header.stamp = path.header.stamp
                    new_pose.pose.position.x, new_pose.pose.position.y, new_pose.pose.position.z = new_point
                    fixed_path.poses.insert(i + j, new_pose)
    ...
    def calculate_path_trav_points(self, position, orientation, head_pos, tail_pos):
        # Copy path
        with self.debug_track_lock:
            path = deepcopy(self.combined_path)
        ...
        with self.trav_vis_pc_lock:
            ...
            if path is not None:
                for i in range(0, len(path.poses), self.config["path_lookahead_pose_skip"]):
                    ...
                    if self.dist_between_2_positions([position.x, position.y], [x,y]) > self.config["max_path_lookahead_dist"]:
                        break
    ...
    def publish_combined_path(self):
        if hasattr(self, "combined_path") and self.combined_path is not None:
            self.combined_path_pub.publish(self.combined_path)
        ...
        with self.mobility_params_lock:
            if hasattr(self, "mobility_params_data"):
                if self.mobility_params_data.stair_hint == cmd_bool:
                    return
                if self.mobility_params_data.locomotion_hint != self.config["HINT_CRAWL"]:
                    self.set_spot_locomotion_type("HINT_AUTO")
    ...
    def set_spot_locomotion_type(self, gait_type):
        ...
        with self.mobility_params_lock:
            if hasattr(self, "mobility_params_data"):
                if self.mobility_params_data.stair_hint or self.mobility_params_data.locomotion_hint == gait_num:
                    return

        rospy.loginfo("Attempting to set gait type: {}".format(gait_type))
        ...
        while not rospy.is_shutdown():
            ...
            if len(mean_locs) > 0:
                for mean_loc, xy_dir, slope in zip(mean_locs, mean_slope_dirs, slopes):
                    if abs(slope) >= self.config["min_slope_for_dir_enforcement"] and self.dist_between_2_positions([position.x, position.y], mean_loc[:2]) < self.config["dir_enforcement_dist_threshold"]:
                        if slope < 0:
                            self.walking_dir = "FW"
                        else:
                            self.walking_dir = "BW"
            ...
            high_walking_slope = False
            if len(mean_locs) > 0:
                for mean_loc, slope in zip(mean_locs, slopes):
                    if 10 > abs(slope) > self.config["min_slope_for_dir_enforcement"] and self.dist_between_2_positions([position.x, position.y], mean_loc[:2]) < self.config["dir_enforcement_dist_threshold"]:
                        high_walking_slope = True
                        break
            ...
            #gait_type = "HINT_AUTO"
            #if len(mean_travs) > 0:
            #    for mean_trav, mean_loc in zip(mean_travs, mean_locs):
            #        difficult_obst = mean_trav > self.config["mean_trav_crawl_thresh"]
            #        is_close = self.dist_between_2_positions([position.x, position.y], mean_loc[:2]) < self.config["crawl_enforcement_dist_threshold"]
            ...
            with self.nav_cmd_vel_lock:
                if hasattr(self, "nav_cmd_vel_data"):
                    if self.nav_cmd_vel_data is not None:
                        if self.config["crawl_vel_thresh_low"] < abs(self.nav_cmd_vel_data.linear.x) < self.config["crawl_vel_thresh_high"]:
                            low_cmd_vel = True
                            rospy.loginfo_throttle(1, "Spot assistant registered low cmd vel, might attempt to set to crawl mode")
            ...
            if self.crawl_mode_on or high_walking_slope or low_cmd_vel:
                rospy.loginfo_throttle(1, "Spot assistant registered hint_crawl")
                gait_type = "HINT_CRAWL"
