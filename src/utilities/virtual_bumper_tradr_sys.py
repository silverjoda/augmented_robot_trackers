...
class VirtualBumper:
    ...
    def init_ros(self, name):
        ...
        self.laser_semantic_data = None
        self.cmd_vel_in_data = None

        self.cmd_vel_publisher = rospy.Publisher("nav/cmd_vel", #cmd_vel_desired_speed
                                                 Twist,
                                                 queue_size=1)
        ...
        self.laser_semantic_lock = threading.Lock()
        self.cmd_vel_in_lock = threading.Lock()

        self.cmd_vel_new = False

        rospy.Subscriber("points_filtered",
                         PointCloud2,
                         self._ros_laser_callback, queue_size=3)

        rospy.Subscriber(self.config["semantic_pc_name"],
                         PointCloud2,
                         self._ros_laser_semantic_callback, queue_size=3)

        rospy.Subscriber(self.config["cmd_vel_in"],
                         Twist,
    ...
    def _ros_laser_semantic_callback(self, data):
        with self.laser_semantic_lock:
            self.laser_semantic_data = data
    ...
    def check_and_tick_new_cmd_vel(self):
        with self.cmd_vel_in_lock:
            if self.cmd_vel_new:
                self.cmd_vel_new = False
                return True
    ...
    def loop_virtual_bumper(self):
        while not rospy.is_shutdown():
            # If no new cmd_vel received, then wait
            #if not self.check_and_tick_new_cmd_vel():
            #    time.sleep(0.05)
            #    continue
    ...
    def get_bumper_activations(self, target_lin, target_ang):
        ...
        with self.laser_lock:
            if self.laser_data is None: return bumper_activation_dict

        #if self.config["use_semantic_info"]:
        #    pc = self.get_laser_semantic_in_baselink()
        #else:
    ...
    def get_laser_in_baselink(self):
        ...
        if self.config['use_semantic_info']:
            with self.laser_semantic_lock:
                pc = ros_numpy.numpify(self.laser_semantic_data).ravel()
                source_frame = self.laser_semantic_data.header.frame_id
            pc = pc[pc['obstacle'] > 0]
        else:
        ...
        #print(pc_array_clean[2, np.random.randint(0,50000, 100)])
        
        # Transform pc to baselink coordinates
        try:
            trans = self.tf_buffer.lookup_transform("base_link",  # base_link
                                                    source_frame,
                                                    rospy.Time(0),
                                                    rospy.Duration(0))
    ...
    def get_laser_semantic_in_baselink(self):
        # Get local point cloud
        with self.laser_semantic_lock:
            pc = ros_numpy.numpify(self.laser_semantic_data).ravel()
            source_frame = self.laser_semantic_data.header.frame_id
        ...
        # Array of size (4, N_points). 4th coordinate is for homogeneous transformation
        pc_array = np.stack([pc[f] for f in ['x', 'y', 'z', 'obstacle']] + [np.ones(pc.size)])
        not_nan_indeces = np.logical_not(np.any(np.isnan(pc_array), axis=0))
        pc_array_clean = pc_array[:, not_nan_indeces]
        

        # Filter array according to obstacle field. If 1 then obstacle, 0 is trav.
        # Also choose just the xyz and ones fields
        #pc_array_trav = pc_array_clean[[0,1,2,4], pc_array_clean[3, :] < 1]
        obst_indeces = pc_array_clean[3, :] > 0
        pc_array_trav = pc_array_clean[[0,1,2,4]]
        pc_array_trav = pc_array_trav[:, obst_indeces]
        ...
        try:
            ...
            rospy.logwarn("Transforming laser semantic to base_link in virtual bumper failed. ", err)
            return None

        pc_baselink = np.matmul(trans, pc_array_trav)[:3, :]
