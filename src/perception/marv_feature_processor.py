...
class MarvFeatureProcessor:
    ...
    def init_ros(self):
        rospy.init_node("marv_feature_processor")
        ...
        self.stagnation_publisher = rospy.Publisher("art/marv_progress_stagnation",
                                                    Float64,
                                                    queue_size=1)
        ...
        self.track_vel_subscriber = ros_utilities.subscriber_factory("X1/cmd_vel", Twist)
    ...
    def get_all_observation_dict(self):
        pose_dict = ros_utilities.get_robot_pose_dict(self.config["root_frame"], "X1/base_link", self.tf_buffer, rospy.Time(0))
    ...
    def get_exteroceptive_data_dict(self, pose_dict):
        ...
        height_offset = -pose_dict["euler"][1] * 0.2
    ...
    def make_dataset(self, n_secs, suffix=None):
        ...
        if suffix is not None:
            dataset_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "envs/supervised_dataset/sm_{}".format(suffix))
...
def main():
    ...
    if len(myargv) == 2:
        ...
        config_name = "marv_feature_processor_config.yaml"

    import yaml
    with open(os.path.join(os.path.dirname(__file__), "configs/{}".format(config_name)), 'r') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)

    fp = MarvFeatureProcessor(config)
    fp.loop()
    #fp.make_dataset(600, "logs")
