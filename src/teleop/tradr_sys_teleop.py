...
class TradrTeleop:
    def __init__(self, config):
        ...
        rospy.loginfo("Starting tradr teleop in {} mode".format(self.control_mode))
    ...
    def init_ros(self, name):
        ...
        self.tracks_vel_publisher = rospy.Publisher("/cmd_vel",
                                                    Twist,
                                                    queue_size=1)
        ...
        rospy.loginfo("Tradr teleop node {}: initialized ros".format(self.config["node_name"]))
    ...
    def _process_waypoints(self, joy_data):
        ...
        try:
            ...
            rospy.logwarn_throttle(1, "Tradr teleop: process waypoints: TRANSFORMATION OLD, err: {}".format(err))
            return [], 0
    ...
    def publish_commands(self, commands, publish_flippers=True, publish_tracks=True):
        ...
        if publish_flippers:
            self.flippers_pos_fl_publisher.publish(Float64(data=fl_flipper_pos))
            self.flippers_pos_fr_publisher.publish(Float64(data=fr_flipper_pos))
            self.flippers_pos_rl_publisher.publish(Float64(data=rl_flipper_pos))
            self.flippers_pos_rr_publisher.publish(Float64(data=rr_flipper_pos))
...
def main():
    ...
    if len(myargv) == 2:
        ...
        config_name = "tradr_sys_teleop_config.yaml"
    ...
    tracker = TradrTeleop(tracker_config)
    tracker.loop()
