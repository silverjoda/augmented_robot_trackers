...
from src.envs.marv_dataset_flipper_env import MarvDatasetFlipperEnv
...
class SupervisedSMFlipperEnvTrainer:
    ...
    def train_differentiable_sm(self):
        ...
        T.save(policy.state_dict(), "agents/{}_marv/imitation_dsm_{}.p".format(self.config["data_dir_name"], self.policy_suffix))
    ...
    def train_differentiable_detached_sm(self):
        ...
        T.save(policy.state_dict(), "agents/{}_marv/imitation_dsm_detached_{}.p".format(self.config["data_dir_name"], self.policy_suffix))
    ...
    def train_state_classification(self):
        ...
        T.save(policy.state_dict(), "agents/{}_marv/imitation_state_classification_{}.p".format(self.config["data_dir_name"], self.policy_suffix))
    ...
    def train_reimp(self):
        ...
        T.save(policy.state_dict(), "agents/{}_marv/imitation_reimp_{}.p".format(self.config["data_dir_name"], self.policy_suffix))
    ...
    def test_differential_sm(self):
        policy = DSM(feat_dim=self.config["feat_vec_dim"],
                     state_transition_dict=self.env.state_transition_dict,
                     initial_state="N", linear=self.config["linear"])
        policy.load_state_dict(T.load("agents/{}_marv/imitation_dsm_{}.p".format(self.config["data_dir_name"], self.policy_suffix)), strict=False)
    ...
    def test_differential_detached_sm(self):
        policy = DSM(feat_dim=self.config["feat_vec_dim"],
                     state_transition_dict=self.env.state_transition_dict,
                     initial_state="N", linear=self.config["linear"])
        policy.load_state_dict(T.load("agents/{}_marv/imitation_dsm_detached_{}.p".format(self.config["data_dir_name"], self.policy_suffix)), strict=False)
    ...
    def test_state_classification(self):
        policy = StateClassifier(feat_dim=self.config["feat_vec_dim"], state_list=self.env.state_list, linear=self.config["linear"])
        policy.load_state_dict(T.load("agents/{}_marv/imitation_state_classification_{}.p".format(self.config["data_dir_name"], self.policy_suffix)), strict=False)
    ...
    def test_reimp(self):
        policy = StateClassifier(feat_dim=13, state_list=self.env.state_list, linear=self.config["linear"])
        policy.load_state_dict(T.load("agents/{}_marv/imitation_reimp_{}.p".format(self.config["data_dir_name"], self.policy_suffix)), strict=False)
...
def main(trainer_config, max_datasets=100):
    env = MarvDatasetFlipperEnv(max_datasets=max_datasets, dir_name=trainer_config["data_dir_name"])
...
if __name__=="__main__":
    import yaml
    with open(os.path.join(os.path.dirname(__file__), "configs/train_sm_flipper_env_supervised.yaml"), 'r') as f:
