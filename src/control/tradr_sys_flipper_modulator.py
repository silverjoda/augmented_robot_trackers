...
from nifti_robot_driver_msgs.msg import FlippersState, FlippersTorque, FlippersVel
...
class TradrFlipperModulator:
    ...
    def init_ros(self):
        ...
        self.flippers_pos_publisher = rospy.Publisher("/flippers_cmd",
                                                         FlippersState,
                                                         queue_size=1)

        self.flippers_max_vel_publisher = rospy.Publisher("/flippers_max_vel_cmd",
                                                      FlippersVel,
                                                      queue_size=1)

        self.flippers_torque_publisher = rospy.Publisher("/set_flippers_torque",
                                                            FlippersTorque,
                                                            queue_size=1)
        ...
        self.imu_subscriber = ros_utilities.subscriber_factory("imu/data", Imu)
    ...
    def publish_flipper_pos(self, flipper_dict):
        ...
        msg = FlippersState()
        msg.frontLeft = flippers_pos_cmd_data[0]
        msg.frontRight = flippers_pos_cmd_data[1]
        msg.rearLeft = flippers_pos_cmd_data[2]
        msg.rearRight = flippers_pos_cmd_data[3]
        self.flippers_pos_publisher.publish(msg)
    ...
    def publish_flipper_torque_limits(self, flipper_dict):
        ...
        if self.current_base_link_frame == self.config["robot_prefix"] + "base_link_zrp":
            ...
            flippers_torque_data = [flipper_dict["rear_right"], flipper_dict["rear_left"],
                                     flipper_dict["front_right"], flipper_dict["front_left"]]
        msg = FlippersTorque()
        msg.front = flippers_torque_data[0]
        msg.rear = flippers_torque_data[2]
        self.flippers_torque_publisher.publish(msg)

    def publish_flippers_max_vel(self):
        # Sex maximum flippers vel
        vel_msg = FlippersVel()
        vel_msg.frontLeft = 2.0
        vel_msg.frontRight = 2.0
        vel_msg.rearLeft = 2.0
        vel_msg.rearRight = 2.0
        self.flippers_max_vel_publisher.publish(vel_msg)
...
def main():
    ...
    if len(myargv) == 2:
        ...
        config_name = "tradr_sys_flipper_modulator_config.yaml"
