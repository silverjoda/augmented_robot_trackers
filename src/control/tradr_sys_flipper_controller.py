...
class TradrFlipperController:
    ...
    def init_ros(self):
        ...
        self.tracker_state_publisher = rospy.Publisher("art/tradr_flipper_controller_state",
                                                 String,
                                                 queue_size=1)
...
def main():
    ...
    if len(myargv) == 2:
        ...
        config_name = "tradr_sys_flipper_controller_config.yaml"

    import yaml
    with open(os.path.join(os.path.dirname(__file__), "configs/{}".format(config_name)), 'r') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)

    controller = TradrFlipperController(config)
    controller.loop()
